...
### 自动生成输出文件名
当命令行未指定输出文件名时，程序会根据以下规则自动生成输出文件名：
1. 如果配置文件中指定了`title`字段，则以指定的title加上`.txt`扩展名作为输出文件名
2. 如果`title`未指定或为空，则查看`.list`文件的第一行：
   - 如果第一行为注释行（以`#`开头），则以第一行去除`#`号和空格作为输出文件名
   - 如果第一行非注释行或内容为空，则将list文件名加`.txt`作为输出文件名
3. 如果既没有配置文件也没有list文件，则使用默认文件名`output.txt`
...
# 自动生成输出文件名（省略output_file参数）
python netreptile.py --list urls.txt --config config.json
python netreptile.py --list urls.txt
```
...
### 示例3：自动生成输出文件名
1. 使用现有的配置文件 `config/6park.json`（包含title字段："《大唐游记》"）：
```json
{
    "timeout": 50000,
    "selectors": [
        {
            "selector": ".show_content",
            "selector_type": "css",
            "separator": "\n\n"
        }
    ],
...
2. 使用现有的URL列表文件 `url_list/大唐游记.list`（第一行是注释："# 大唐游记"）：
```
# 大唐游记
<a href="https://www.cool18.com/bbs4/index.php?app=book&act=bookview&cid=3885">【大唐游记】 1</a>
<a href="https://www.cool18.com/bbs4/index.php?app=book&act=bookview&cid=3886">【大唐游记】 2</a>
```

3. 运行命令（省略output_file参数）：
```bash
# 使用配置文件，程序将根据config中的title生成输出文件名
python netreptile.py --list url_list/大唐游记.list --config config/6park.json

# 不使用配置文件，程序将根据list文件第一行注释生成输出文件名
python netreptile.py --list url_list/大唐游记.list

# 不使用配置文件和list文件，程序将使用默认文件名
python netreptile.py https://example.com
```

4. 程序将根据规则生成输出文件名：
   - 第一个命令：使用config中的title，生成"《大唐游记》.txt"
   - 第二个命令：使用list文件第一行注释，生成"大唐游记.txt"
   - 第三个命令：没有config和list文件，生成"output.txt"
